...
async def test_llm_settings(
    ...
    try:
        ...
        return JSONResponse({"status": "success", "message": "Configuration valide. Connexion réussie."})
    except LLMError as e:
        return JSONResponse(
            {"status": "error", "message": f"Échec de l'appel LLM: {str(e)}"},
            status_code=400
        )
    except Exception as e:
        return JSONResponse(
...
async def list_plugins(
    ...
    plugin_db_states = {}
    try:
        async with db.execute("SELECT plugin_id, enabled, config_json FROM plugin_configs") as cursor:
            rows = await cursor.fetchall()
            for row in rows:
                plugin_db_states[row[0]] = {
                    "enabled": bool(row[1]),
...
async def upload_plugin(
    ...
    if not file.filename.endswith(".py"):
        raise HTTPException(status_code=400, detail="Seuls les fichiers Python (.py) sont autorisés.")
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Erreur de syntaxe Python: {str(e)}"
        )
    ...
    return JSONResponse({"status": "success", "message": f"Plugin '{plugin_name}' téléversé et activé avec succès."})
...
async def configure_plugin(
    ...
    return JSONResponse({"status": "success", "message": f"Configuration du plugin '{plugin_id}' mise à jour."})
...
async def toggle_plugin(
    ...
    # Get current state
    enabled = True
    async with db.execute("SELECT enabled FROM plugin_configs WHERE plugin_id = ?", (plugin_id,)) as cursor:
...
async def delete_plugin(
    ...
    if plugin_id in ["metrics", "systemd", "docker"]:
        raise HTTPException(status_code=400, detail="Les extensions intégrées au système ne peuvent pas être supprimées.")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Impossible de supprimer le fichier du plugin : {str(e)}")
    ...
    return JSONResponse({"status": "success", "message": f"Plugin '{plugin_id}' désinstallé avec succès."})
