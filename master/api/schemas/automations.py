...
class AlertCpuHighTrigger(BaseModel):
    """Déclenché quand une alerte CPU haute se déclenche."""
    alert_name: Literal["cpu_high_percent", "cpu_high_load", "cpu_load_per_core_high"] = "cpu_high_percent"


class AlertMemoryHighTrigger(BaseModel):
    """Déclenché quand une alerte mémoire haute se déclenche."""
    alert_name: Literal["memory_usage_high"] = "memory_usage_high"


class AlertDiskHighTrigger(BaseModel):
    """Déclenché quand une alerte disque haute se déclenche."""
    alert_name: Literal["disk_usage_high"] = "disk_usage_high"
...
class AutomationRuleCreate(BaseModel):
    ...
    trigger_type: Literal[
        "metric_threshold", "node_state",
        "node_intent_failure", "node_health", "audit_alert",
        "alert_cpu_high", "alert_memory_high", "alert_disk_high",
    ]
