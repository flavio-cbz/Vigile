...
class HeavyProcessConfig(BaseModel):
    ...
    cpu_threshold_percent: float = Field(description="CPU percentage threshold above which this is considered active")
...
class NodeProfile(BaseModel):
    ...
    baseline_ram_percent: float = Field(default=70.0, description="Expected baseline RAM utilization percentage")
    context_label: str = Field(default="Serveur homelab", description="Overall context label of the node")
...
class InsightsManager:
    ...
    async def generate_profile(
        ...
        if node.get("online"):
            ...
            if not containers:
                try:
                    ...
                    result = await nm.send_intent(node_id, {"action": "LIST_CONTAINERS"}, timeout=8.0)
                    if result.get("success"):
        ...
        profile = None
        if self._llm_client and self._llm_client.base_url:
            lang_instruction = (
            ...
            try:
                ...
                logger.warning("LLM profile generation failed for node %s: %s. Using local fallback.", node_id, ex)

        # 4. Fallback to local heuristic rules
        if not profile:
            profile = self.generate_fallback_profile(node_id, hostname, os_name, services, containers)
    ...
    def generate_fallback_profile(
        ...
        for c in containers:
            ...
            if "plex" in c_name or "jellyfin" in c_name:
                known_heavy.append(HeavyProcessConfig(container_name=c.get("name"), cpu_threshold_percent=50.0, label="Transcodage Multimédia (Plex/Jellyfin)"))
            elif "postgres" in c_name or "pg-" in c_name:
                known_heavy.append(HeavyProcessConfig(container_name=c.get("name"), cpu_threshold_percent=40.0, label="Requêtes Base de Données (PostgreSQL)"))
            elif "mysql" in c_name or "mariadb" in c_name:
                known_heavy.append(HeavyProcessConfig(container_name=c.get("name"), cpu_threshold_percent=40.0, label="Requêtes Base de Données (MySQL)"))
            elif "nginx" in c_name or "proxy" in c_name or "traefik" in c_name:
                known_heavy.append(HeavyProcessConfig(container_name=c.get("name"), cpu_threshold_percent=30.0, label="Trafic Web (Reverse Proxy)"))
            elif "runner" in c_name or "gitlab" in c_name or "jenkins" in c_name:
                known_heavy.append(HeavyProcessConfig(container_name=c.get("name"), cpu_threshold_percent=60.0, label="Compilation / Pipeline CI-CD"))

        # Analyze systemd services
        for s in services:
            s_name = s.get("name", "").lower()
            if "plex" in s_name:
                known_heavy.append(HeavyProcessConfig(service_name=s.get("name"), cpu_threshold_percent=50.0, label="Transcodage Plex Media Server"))
            elif "postgresql" in s_name or "mysql" in s_name:
                known_heavy.append(HeavyProcessConfig(service_name=s.get("name"), cpu_threshold_percent=45.0, label="Activité Base de Données"))
            elif "nginx" in s_name or "apache" in s_name:
                known_heavy.append(HeavyProcessConfig(service_name=s.get("name"), cpu_threshold_percent=35.0, label="Pic de Trafic Web"))
            elif "fail2ban" in s_name:
                known_heavy.append(HeavyProcessConfig(service_name=s.get("name"), cpu_threshold_percent=20.0, label="Analyse d'intrusions Fail2ban"))
        ...
        elif "plex" in host_lower or "media" in host_lower or "nas" in host_lower or any("plex" in c.get("name", "").lower() for c in containers):
    ...
    def _calculate_cpu_insight(
        ...
        if cached_containers:
            try:
                active_containers = [
                    c.get("name") for c in json.loads(cached_containers)
        ...
        if cached_services:
            try:
                active_services = [
                    s.get("service") for s in json.loads(cached_services)
    ...
    def _calculate_ram_insight(self, latest_snap: dict[str, Any], profile: NodeProfile) -> dict[str, Any]:
        ...
        if mem_percent > 90:
            ...
            detail = "Aucune pression d'échange (swap)" if swap_mb < 50 else f"Swap utilisé : {swap_mb:.0f} Mo"
