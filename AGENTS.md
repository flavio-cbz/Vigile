...
- **Worker Self-Update & Canary Rollout**: The Go Worker features a fail-safe auto-update lifecycle. At startup, `updater.CheckAndRollbackIfFailed` checks for a pending update; if repeated crashes (>= 5 attempts) or a 60s connection timeout occur, it automatically restores the previous binary (`execPath.previous` or `DefaultPreviousLink`) and exits to let the supervisor restart the proven version. Upon entering operational WebSocket phase, `updater.ConfirmUpdate()` clears the pending state. The Master's `auto_update_workers_task` orchestrates Canary deployments (updating and validating a single node before fleet rollout), performs semver parsing (`is_newer_version`), and logs each step to the audit hash chain (`AuditAction.AUTO_UPDATE_WORKER`).
- **Embedded Worker Distribution in Dockerfile.master**: Multi-stage Docker build includes `worker-builder` (`golang:1.23-alpine`) cross-compiling static Linux binaries (`worker-linux-amd64`, `worker-linux-arm64`) into `master/static/releases/`, guaranteeing local and offline availability without depending on external GitHub releases. Fallback logic in `master/api/worker_binary.py` dynamically synthesizes manifests from local releases.
- **Worker Concurrency Pools & Fail-Fast (Ticket Core-Robustesse)**: The Go Worker enforces strict concurrency isolation: read-only queries run up to 4 concurrently (`querySem`), mutations are strictly serialized with capacity 1 (`mutationSem`), and disk scans are serialized with capacity 1 (`diskScanSem`). When a mutation or disk scan is already in progress, the worker rejects incoming conflicting intents immediately with `WORKER_BUSY`.
- **Session Epoch Tracking & Instant Socket Close (Ticket Core-Robustesse)**: `WorkerConn.sessionEpoch` increments on every operational WebSocket session. Outgoing frames verify `wc.sessionEpoch.Load() == epoch` before transmission, eliminating stale cross-session frame leaks. `WSConn.Close()` uses `atomic.Bool` to unblock readers and closes the network socket instantly (0ms) without acquiring `ws.mu`.
- **Metrics Buffering & Zero-SQL Read Cache (Ticket Core-Robustesse)**: Telemetry snapshots from `STATUS_REPORT` are buffered in an in-memory ring buffer (`_metrics_buffer`) and flushed to SQLite in batches using `executemany` every 5s or upon accumulating 25 snapshots. `NodeManager._latest_metrics` provides a zero-SQL in-memory cache for fast status checks, cache updates, and proposal auto-expiry.
