...
def ensure_virtual_env() -> str:
    ...
    if not venv_python:
        ...
        try:
            subprocess.run(
                [sys.executable, "-m", "venv", os.path.join(PROJECT_DIR, ".venv")], check=True
            )
...
def ensure_frontend_dependencies() -> None:
    ...
    vite_bin = os.path.join(
        node_modules_dir, ".bin", "vite.cmd" if sys.platform == "win32" else "vite"
    )
    if not os.path.exists(node_modules_dir) or not os.path.exists(vite_bin):
        print(
            "📦 Dépendances frontend (Vite) manquantes ou incomplètes. Installation/réparation en cours..."
        )
        npm_cmd = "npm.cmd" if sys.platform == "win32" else "npm"
        if not shutil.which(npm_cmd):
            print(
                "❌ Erreur: 'npm' n'est pas installé ou n'est pas disponible dans votre variable PATH."
            )
...
def main() -> None:
    ...
    if run_frontend and is_port_in_use(args.host, int(args.frontend_port)):
        if not kill_process_on_port(int(args.frontend_port)):
            print(
                f"❌ Erreur: Le port frontend {args.frontend_port} est déjà utilisé sur {args.host}."
            )
            print(
                "Libérez le port ou utilisez `--frontend-port <port>` pour spécifier un autre port."
            )
            sys.exit(1)
