...
async def test_login_invalid_credentials(client: AsyncClient):
    ...
    response_nonexistent = await client.post(
        "/api/auth/login", json={"username": "nonexistent", "password": "anypassword"}
    )
...
async def test_login_inactive_user(client: AsyncClient, db, security: SecurityManager):
    ...
    response = await client.post(
        "/api/auth/login", json={"username": "inactive_user", "password": "password123"}
    )
...
async def test_refresh_token_success(client: AsyncClient, db):
    ...
    async with db.execute("SELECT action FROM audit_log WHERE action = 'TOKEN_REFRESH'") as cursor:
        row = await cursor.fetchone()
        assert row is not None
...
async def test_refresh_token_theft_detection(client: AsyncClient, db):
    ...
    refresh_resp_1 = await client.post("/api/auth/refresh", json={"refresh_token": refresh_token})
    assert refresh_resp_1.status_code == status.HTTP_200_OK

    # 2. Use same token again (theft detection triggers!)
    refresh_resp_2 = await client.post("/api/auth/refresh", json={"refresh_token": refresh_token})
...
async def test_login_password_verify_exception(client: AsyncClient, security: SecurityManager):
    ...
    with mock.patch.object(
        security, "verify_password", side_effect=Exception("verification error")
    ):
        response = await client.post(
            "/api/auth/login", json={"username": "nonexistent", "password": "anypassword"}
        )
...
@pytest.mark.asyncio
async def test_refresh_token_user_not_found_or_inactive(
    client: AsyncClient, db, security: SecurityManager
):
    ...
    response2 = await client.post("/api/auth/refresh", json={"refresh_token": token2})
...
async def test_logout_with_invalid_token(client: AsyncClient):
    response = await client.post("/api/auth/logout", json={"refresh_token": "invalid_token_string"})
