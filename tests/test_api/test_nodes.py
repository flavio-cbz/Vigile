...
async def test_generate_join_token_success(client: AsyncClient, db, auth_headers):
    ...
    async with db.execute("SELECT consumed, expires_at FROM join_tokens WHERE token_hash = ?", (token_hash,)) as cursor:
        row = await cursor.fetchone()
        assert row is not None
...
async def test_list_nodes(client: AsyncClient, db, auth_headers):
    ...
    # Filtering by state
    response_filtered = await client.get("/api/nodes?state=CONNECTED", headers=auth_headers("operator"))
...
async def test_get_node_stats(client: AsyncClient, db, auth_headers):
    ...
    # Non-existent node -> 404
    response_404 = await client.get("/api/nodes/nonexistent/stats", headers=auth_headers("operator"))
    assert response_404.status_code == status.HTTP_404_NOT_FOUND
...
async def test_verify_chain(client: AsyncClient, auth_headers):
    ...
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert "verified" in data or "valid" in data or "report" in data or "success" in data or "corrupted" in data
...
async def test_get_node_logs(client: AsyncClient, db, auth_headers):
    ...
    try:
        response = await client.get("/api/nodes/n-6/logs?lines=50", headers=auth_headers("operator"))
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
