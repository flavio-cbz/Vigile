...
async def test_service_status_success(db, client, auth_headers):
    ...
    try:
        resp = await client.get(f"/api/nodes/{node_id}/services/nginx.service", headers=auth_headers("admin"))
        assert resp.status_code == 200
        d = resp.json()
        assert d["node_id"] == node_id
...
async def test_service_status_fail(db, client, auth_headers):
    ...
    try:
        resp = await client.get(f"/api/nodes/{node_id}/services/unknown.service", headers=auth_headers("admin"))
        assert resp.status_code == 200
        d = resp.json()
...
async def test_restart_service_success(db, client, auth_headers):
    ...
    try:
        resp = await client.post(f"/api/nodes/{node_id}/services/nginx.service/restart", headers=auth_headers("admin"))
        assert resp.status_code == 200
        d = resp.json()
        assert d["node_id"] == node_id
        assert d["service"] == "nginx.service"
...
async def test_restart_service_admin_required(db, client, auth_headers):
    ...
    node_manager.send_intent = mock_restart_service
    try:
        resp = await client.post(f"/api/nodes/{node_id}/services/nginx.service/restart", headers=auth_headers("operator"))
...
async def test_restart_container_success(db, client, auth_headers):
    ...
    try:
        resp = await client.post(f"/api/nodes/{node_id}/containers/a1b2c3d4e5f6/restart", headers=auth_headers("admin"))
        assert resp.status_code == 200
        d = resp.json()
        assert d["node_id"] == node_id
