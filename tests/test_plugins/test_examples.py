...
async def test_clean_logs_plugin(db: aiosqlite.Connection):
    ...
    await db.execute(
        "INSERT OR IGNORE INTO plugin_configs (plugin_id, enabled, config_json) VALUES (?, 1, ?)",
        ("clean_logs", json.dumps({
            "disk_threshold": 75,
            "cleanup_patterns": "/var/log/*.1"
        }))
    )
    ...
    async with db.execute("SELECT reasoning, params_json FROM action_proposals WHERE node_id = 'test-node-clean' AND status = 'PENDING'") as cursor:
