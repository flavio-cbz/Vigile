/**
 * Vigile — Centralized HTTP API Client
 *
 * Core fetch wrapper with automatic JWT Bearer header injection,
 * session refresh on 401, retry with exponential backoff, request
 * timeouts, and toast-based error reporting.
 *
 * Extracted from hooks/useApi.ts (WP8 god-file split).
 */
    ...
    try {
      ...
      if (
        ...
        useToastStore
          ...
          .addToast(
            'error',
            t('api.toast.session_expired'),
            t('api.toast.session_expired_msg'),
          );
        return null;
      }
      ...
      if (!response.ok) {
        if (response.status === 429) {
          // Rate-limited request — deduplicate toasts with a cooldown
          const errorText = await response
            .text()
            .catch(() => t('common.error_unknown'));
          ...
          if (!skipToast) {
            const now = Date.now();
            if (
              now - lastRateLimitToastTime > RATE_LIMIT_TOAST_COOLDOWN_MS
            ) {
        ...
        try {
          ...
          if (parsed && typeof parsed === 'object') {
            displayMessage =
              parsed.detail || parsed.message || parsed.error || errorText;
          }
        } catch {
      ...
      if (
        !skipToast &&
        !(normalizedError instanceof Error && toastedErrors.has(normalizedError))
      ) {
