...
import { INTENT_ACTIONS, type FormAction, type ActionType } from './ruleFormTypes';

interface ActionConfigProps {
  actions: FormAction[];
  onAdd: () => void;
  onRemove: (index: number) => void;
  onUpdate: (index: number, updates: Partial<FormAction>) => void;
}

export const ActionConfig: React.FC<ActionConfigProps> = ({
  actions, onAdd, onRemove, onUpdate,
}) => {
  return (
        ...
        <div key={i} className="border border-border rounded-lg p-3 space-y-2 bg-surface-2">
          <div className="flex items-center gap-2">
            <select
              ...
              value={action.type}
              onChange={e => onUpdate(i, { type: e.target.value as ActionType })}
            >
