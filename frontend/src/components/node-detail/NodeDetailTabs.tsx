...
export const useNodeDetailTabs = (params: {
  ...
  const { t } = useLocale();
  const { insightsCount, servicesCount, servicesLoading, containersCount, containersLoading } = params;
  return [
    { id: 'insights', label: t('node_detail.tab.insights'), count: insightsCount },
    { id: 'metrics', label: t('node_detail.tab.metrics') },
    { id: 'services', label: t('node_detail.tab.services', { count: servicesLoading ? '•' : servicesCount }) },
    { id: 'containers', label: t('node_detail.tab.containers', { count: containersLoading ? '•' : containersCount }) },
