...
export const NodeSelector: React.FC = () => {
  ...
  return (
    <div ref={dropdownRef} className="relative select-none font-interface">
      ...
      {isOpen && (
        <div className="absolute left-0 mt-2 w-56 rounded-lg bg-surface-2/95 backdrop-blur-xs border border-border-strong/60 shadow-[var(--shadow-dropdown)] py-1.5 z-50 animate-fade-in">
          <button
            onClick={() => {
              selectNode('all');
              navigate('/');
