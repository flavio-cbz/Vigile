...
import type { IncidentPeriod } from './trendUtils';

interface TrendNodeRowProps {
  node: Node;
  nodeIdx: number;
  ...
  setHoveredBar: (hover: { nodeIdx: number; barIdx: number } | null) => void;
}

export const TrendNodeRow: React.FC<TrendNodeRowProps> = ({
  node, nodeIdx, timelineBars, uptimePct, nodeIncidents,
  period, t, hoveredBar, setHoveredBar,
}) => {
  return (
        ...
        <div className="flex items-center justify-between gap-1 py-1.5">
          {timelineBars.map((bar, barIdx) => (
            <TrendBar
              ...
              onHover={() => setHoveredBar({ nodeIdx: nodeIdx, barIdx })}
              onLeave={() => setHoveredBar(null)}
            />
          ))}
        </div>

        <ChartAxis period={period} t={t} />
      </div>
